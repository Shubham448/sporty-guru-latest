  ...
  switch (method) {
    case 'POST':
      try {
        ...
        if (!trainerRecord) return makeResponse(res, code.NOT_FOUND, false, message.TRAINER_NOT_FOUND);
        let verifyPassword = await matchPassword(body.password, trainerRecord.password);
        if (!verifyPassword) return makeResponse(res, code.BAD_REQUEST, false, message.INCORRECT_PASSWORD);
